...
public static class DateTimeHelper {
    ...
    public static string GetDateString(DateTime date, string formatString) {
        if(date.Date == DateTime.Now.Date)
            return "Bugün"; //return "Today";
        if (date.Date == (DateTime.Now.AddDays(-1)).Date)
            return "Dün"; //return "Yesterday";
...
public static class ScaleHelper {
    ...
    public static string GetCurrencyAbbreviationMask(string text, AbbreviationType type) {
        return string.Format(GetCurrencyPattern(), GetAbbreviationMask(text, type), CultureInfo.CurrentCulture.NumberFormat.CurrencySymbol); //TL para sembolü buradan geliyor.
    }
