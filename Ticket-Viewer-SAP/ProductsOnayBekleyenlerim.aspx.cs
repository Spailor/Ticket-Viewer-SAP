...
public partial class Products : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        var list = db.S_Destec_CagriOnayBekleyenlerim(-1, userId).ToList();
    ...
    private void CagriDetaylariniYukle(int _ID)
    {
        int id = 0;
        id = _ID;
        var list = db.S_Destec_CagriOnayBekleyenlerim(id, userId).ToList();
        if (list.Count > 0)
        {
            SirketKodu.Text = "Yok";
    ...
    protected void callbackPopupEvet_Callback(object source, CallbackEventArgs e)
    {
        ...
        var cagriisteklist = db.S_Destec_Cagri(id).ToList();//db.S_Destec_CagriIstekIDVer(id).ToList();
        if (cagriisteklist.Count > 0)
        {
            ...
            string icerik = id + " numaralı çağrının test onayı verilmiştir. <br /> İşlem detaylarına 'Kapanmış Çağrılarım' ekranından bakabilirsiniz.";
            ...
            var sid = db.S_Destec_SirketIDVer(cagriisteklist.FirstOrDefault().IstekSirketAdi).ToList();
            Guid userid = new Guid(Membership.GetUser().ProviderUserKey.ToString());
            Guid sirketid = sid.FirstOrDefault().SirketId;
            var alicikisiler = db.S_Destec_GonderilecekMailAdresleri(3, userid, sirketid, -1).ToList().First().MailAdresleri; //"cenk (cenk.yenikoylu@tecs.com.tr);mcy (mcyenikoylu@gmail.com)";
            ...
            foreach (var item in alicikisi)
            {
                ...
                db.I_Mail(-1, mailadresleri, strHTML, false, null, "DESTEC Ticket Viewer <mailservice@tecs.com.tr>", id + " numaralı çağri isteği", "DestecCagriIstegi", false, "");
            }
        }
