...
public partial class Products : BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        var list = db.S_Destec_CagriOnayBekleyen(-1,UserId).ToList();
    ...
    private void CagriDetaylariniYukle(int _ID)
    {
        ...
        id = _ID;
        Guid UserId = new Guid(Membership.GetUser().ProviderUserKey.ToString());
        var list = db.S_Destec_CagriOnayBekleyen(id,UserId).ToList();
        if(list.Count > 0)
        {
            SirketKodu.Text = "Yok";
    ...
    {
        int id = Convert.ToInt32(Session["SessCagriIstekID"]);
        db.U_Destec_CagriTestiniOnayla(id, 2, TxtMemo.Text);
        Session["SessCagriIstekID"] = "";
        Guid UserId = new Guid(Membership.GetUser().ProviderUserKey.ToString());
        var list = db.S_Destec_CagriOnayBekleyen(-1,UserId).ToList();
        ProductsGridView.DataSource = list;
